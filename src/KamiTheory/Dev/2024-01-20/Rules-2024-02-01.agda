...
module KamiTheory.Dev.2024-01-20.Rules-2024-02-01 where
...
data _⊢Type_ : ∀ (Γ : Ctx) -> Γ ⊢Space -> 𝒰₀

private variable
  A : Γ ⊢Type X
  B : Γ ⊢Type Y

data _⊢Var_ : ∀ Γ {X} -> Γ ⊢Type X -> 𝒰₀
data _⊢_ : ∀ Γ {X} -> Γ ⊢Type X -> 𝒰₀

record _⊢TS (Γ : Ctx) : 𝒰₀ where
  inductive
  constructor _under_
  field fst : Γ ⊢Space
  field snd : Γ ⊢Type fst
...
wk-Type : Γ ⊢Type X -> Γ ,[ Y under B ] ⊢Type {!!}
wk-Type = {!!}
...
-- We can interpret a type, space and ⨞ combination as sheaf
sheaf : Γ ⊢ X ⨞ A -> Sheaf (Γ ⊢Open' X) _
sheaf {Γ = Γ} {X = X} {A = A} (loc U) = Restr (Const (Γ ⊢ A)) U
sheaf (F , G) = let FF = sheaf F
                    GG = sheaf G
                in {!!}
